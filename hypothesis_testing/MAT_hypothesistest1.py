...
class run_SIR_Simulation:
    ...
    def run_simulation(self, target=None, mode="targeted"):
        ...
        for i in range(30):
            ...
            top.append(len(history))  # Last time step = epidemic duration
...
class SimSandBox:
    ...
    def SandBox(self):
        run = run_SIR_Simulation(self.network,beta = 0.03, gamma = 0.06, initial_infected = 1, vax_eff = 0.99, vax_fraction = 0.017)
        run.results()
